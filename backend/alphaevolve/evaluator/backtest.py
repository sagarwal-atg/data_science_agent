...
from typing import Any, Sequence, Dict, Optional, Type
...
# Lazy import for loader (requires pwb_toolbox)
_loader_module = None

def _get_loader():
    """Lazy load the ETF loader module."""
    global _loader_module
    if _loader_module is None:
        from alphaevolve.evaluator import loader as _loader_module
...
# INTERNAL HELPERS
# ------------------------------------------------------------------ #
def _load_module_from_code(code: str, name: Optional[str] = None) -> types.ModuleType:
...
def _run_backtest(
    ...
    loader = _get_loader()
    df = loader.load_ohlc(tuple(symbols), start=example_config.START_DATE)
    cerebro = bt.Cerebro()
    loader.add_feeds_to_cerebro(df, cerebro)
